...
def dst_match_addi_filter(commands):
    ...
    while i < len(commands):
        ...
        opecode = command[0]
        if opecode in ['add', 'addi', 'sub', 'subi2', 'and', 'xor', 'shr', 'shri2', 'load', 'pop', 'mov']:
...
def src_match_mov_filter(commands):
    # mov 0 128 r0
    # store r0 0 0
    # ->
    # storei 128 0 0
    ...
    while i < len(commands):
        ...
        opecode = command[0]
        if opecode == 'mov' and command[2] == '0' and command[3] != 'cnt':
            next_command = commands[i+1]
            if next_command[0] in ['add', 'sub', 'and', 'xor', 'shr', 'store', 'push'] and (next_command[1] == command[3]):
                new_commands.append([next_command[0]+'i', command[1], next_command[2], next_command[3]])
                i += 2
                continue
...
def push_mov_filter(commands):
    ...
    while i < len(commands):
        ...
        if opecode in ['push']:
            next_command = commands[i+1]
            if next_command[0] in ['mov', 'addi'] and next_command[3] != command[1] and (command[1] != 'cnt' and next_command[3] != 'cnt'):
